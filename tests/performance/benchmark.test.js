...
describe('Performance Benchmarks', () => {
    describe('Throughput Tests', () => {
        test('should achieve minimum processing speed', async () => {
            const pipeline = StreamingPipeline_1.PipelineFactory.createHighPerformancePipeline();
            ...
            const testContent = test_data_1.TestMarkdown.large; // 20KB+ content
            ...
            expect(charsPerSec).toBeGreaterThan(test_data_1.PerformanceTargets.minCharsPerSec);
        });
        test('should maintain performance with different buffer sizes', async () => {
            ...
            for (const [name, config] of Object.entries(test_data_1.BufferConfigs)) {
                ...
                const duration = Date.now() - startTime;
                results[name] = (test_data_1.TestMarkdown.complex.length / duration) * 1000;
            }
            ...
            Object.values(results).forEach(speed => {
                expect(speed).toBeGreaterThan(test_data_1.PerformanceTargets.minCharsPerSec);
            });
        });
    });
    describe('Latency Tests', () => {
        test('should produce first chunk quickly', async () => {
            ...
            expect(latency).toBeLessThan(test_data_1.PerformanceTargets.maxFirstChunkLatency);
        });
        test('should maintain low average latency between chunks', async () => {
            ...
            expect(avgLatency).toBeLessThan(test_data_1.PerformanceTargets.maxAvgChunkLatency);
        });
    });
    describe('Memory Usage Tests', () => {
        test('should maintain bounded memory usage', async () => {
            const pipeline = StreamingPipeline_1.PipelineFactory.createTextPipeline(test_data_1.BufferConfigs.large);
            pipeline.registerProcessor(new MarkdownProcessor_1.MarkdownProcessor());
            pipeline.registerRenderer(new MarkdownProcessor_1.HTMLRenderer());
            // Process very large content
            const largeContent = test_data_1.TestMarkdown.simple.repeat(1000); // ~50KB
            ...
            expect(memoryPerChar).toBeLessThan(test_data_1.PerformanceTargets.maxMemoryPerChar);
            expect(memoryIncrease).toBeLessThan(test_data_1.PerformanceTargets.maxTotalMemory);
        });
    ...
    describe('Scalability Tests', () => {
        test('should handle multiple concurrent pipelines', async () => {
            ...
            const pipelines = Array.from({ length: pipelineCount }, () => {
                const p = StreamingPipeline_1.PipelineFactory.createTextPipeline();
                p.registerProcessor(new MarkdownProcessor_1.MarkdownProcessor());
                p.registerRenderer(new MarkdownProcessor_1.HTMLRenderer());
                return p;
            });
            const startTime = Date.now();
...
`).join('\n');
    ...
    describe('Stress Tests', () => {
        test('should handle rapid buffer refills', async () => {
            const pipeline = StreamingPipeline_1.PipelineFactory.createTextPipeline(test_data_1.BufferConfigs.tiny); // Small buffers
            pipeline.registerProcessor(new MarkdownProcessor_1.MarkdownProcessor());
            pipeline.registerRenderer(new MarkdownProcessor_1.HTMLRenderer());
        ...
        test('should handle edge case content patterns', async () => {
            ...
            const edgeCases = [
                ...
                test_data_1.TestMarkdown.edge_cases,
                test_data_1.TestMarkdown.unicode
            ];
