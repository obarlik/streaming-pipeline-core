...
const CircularStreamBuffer_1 = require("../../src/framework/CircularStreamBuffer");
const test_data_1 = require("../fixtures/test-data");
describe('CircularStreamBuffer', () => {
    describe('Basic Operations', () => {
        let buffer;
        beforeEach(() => {
            buffer = new CircularStreamBuffer_1.CircularStreamBuffer(64, 128);
        });
        ...
        test('should fill and peek data correctly', () => {
            buffer.fill(test_data_1.TestBinary.simple);
    ...
    describe('Lookahead/Lookbehind', () => {
        let buffer;
        beforeEach(() => {
            buffer = new CircularStreamBuffer_1.CircularStreamBuffer(10, 20);
            buffer.fill(test_data_1.TestBinary.simple);
        });
    ...
    describe('Memory Management', () => {
        test('should auto-compact when lookbehind exceeds limit', () => {
            const buffer = new CircularStreamBuffer_1.CircularStreamBuffer(5, 10); // Small lookBehind
        ...
        test('should handle buffer refill correctly', () => {
            const buffer = new CircularStreamBuffer_1.CircularStreamBuffer(10, 20);
            // Initial fill
            buffer.fill(test_data_1.TestBinary.simple);
    ...
    describe('Boundary Conditions', () => {
        test('should handle circular buffer wrapping', () => {
            const buffer = new CircularStreamBuffer_1.CircularStreamBuffer(5, 10);
...
describe('TextCircularBuffer', () => {
    describe('Text-Specific Operations', () => {
        let buffer;
        beforeEach(() => {
            buffer = new CircularStreamBuffer_1.TextCircularBuffer(64, 128);
        });
        test('should handle UTF-8 text correctly', () => {
            buffer.fillString(test_data_1.TestMarkdown.unicode);
        ...
        test('should handle empty and whitespace strings', () => {
            buffer.fillString(test_data_1.TestMarkdown.whitespace);
            let charCount = 0;
            while (buffer.canAdvance()) {
    ...
    describe('Encoding Handling', () => {
        test('should handle different encodings', () => {
            const buffer = new CircularStreamBuffer_1.TextCircularBuffer(64, 128, 'utf-8');
            buffer.fillString('🎉🚀');
            expect(buffer.peekChar()).toBe('🎉');
            buffer.advance();
            expect(buffer.peekChar()).toBe('🚀');
...
describe('Performance Tests', () => {
    test('should handle large buffers efficiently', () => {
        const buffer = new CircularStreamBuffer_1.CircularStreamBuffer(1024, 2048);
        const startTime = Date.now();
        // Fill with large data
        buffer.fill(test_data_1.TestBinary.large);
        // Perform many operations
        for (let i = 0; i < 1000; i++) {
            buffer.peek();
            buffer.lookAhead(10);
            buffer.lookBehind(10);
            if (buffer.canAdvance())
                buffer.advance();
        }
    ...
    test('should maintain constant memory usage', () => {
        const buffer = new CircularStreamBuffer_1.CircularStreamBuffer(100, 200);
